...
かつて [ヤフーの検索連動広告から京都銀行の偽サイトに誘導される](https://www.nikkei.com/article/DGXNASDG2105Z_R20C14A2CR8000/) というニュースがありましたが、広告経由のフィッシング詐欺のみならず、フィッシング関連ニュースが後を絶ちません。IPA の [情報セキュリティ 10 大脅威 2022](https://www.ipa.go.jp/security/vuln/10threats2022.html) によれば「フィッシングによる個人情報等の詐取」が個人部門の第一位となっています。
...
とはいえ、フィルタ機能などキャリア側の努力でスミッシングの被害は減少していることは朗報です。
...
などがあります。インターネット広告の場合は AI や人手による審査でフィッシングサイトへの誘導を含む悪質なものは除去するのですが、攻撃側も審査をすり抜けるためにあの手この手を使ってきます（なので、ヤフー広告でも審査テクノロジーの改善に一緒に取り組んでくれるエンジニアを募集しております）。掲示板では「お宝画像はこちら」といった投稿をよく見かけますが、リンク先で待ち構えているのはお宝画像ではなくフィッシングサイトかもしれません。コミュニケーション機能については、最近ヤフオクから [質問機能を使ったフィッシングサイト誘導](https://auctions.yahoo.co.jp/topic/notice/other/post_3333/) についての注意喚起がありましたね。
