...
Python は [共通テンプレート](https://github.com/nakayama-kazuki/202x/blob/main/testenv/scripts/template.py) を利用し、ローカルサンドボックス環境の場合 TCP 5000 アクセス、[プロダクション](https://github.com/nakayama-kazuki/202x/tree/main/.github/workflows) では Lambda（zip 形式 + CloudFront ～ Lambda 関数直結）経由のアクセスを想定する。
