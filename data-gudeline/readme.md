...
受領側が個人関連情報を個人データとして取得する場合、ユーザー同意が必要になる点（表の右下）に着目してください。改正個人情報保護法の施行（2022 年 4 月）以前は、個人関連情報 … 例えばウェブサイトの閲覧情報の授受に関する規制はありませんでした。この場合、ユーザーは自身の知らないところで 3rd-party Cookie 紐づけられたウェブサイトの閲覧情報によってスコアリングされ、スコア次第で社会的に不利な扱いを受けてしまう、といった事案（[参考](https://www.ppc.go.jp/files/pdf/191204_houdou.pdf)）が発生するかもしれず、こうした背景から個人関連情報を個人データとして取得する際にはユーザー同意の取得が必要になりました。

記録については（３）や（６）のような第三者提供が増えてゆく場合、記録の内容、記録の保持期間、そして開示請求への対応も踏まえ社内プラットフォームとして仕組化することをお勧めします。
...
ユーザーの権利利益を侵害しないため、ユーザー体験を悪化させないために配慮すべきことはいろいろとあります。

- 位置情報は慎重に扱う<br />（例えば病院や介護施設など、人がどこに所在するかという情報から個人の健康状態や思想信条などが推測できてしまう場合がある）
- その他センシティブな情報も慎重に扱う<br />（要配慮個人情報や、経済状況、人間関係など）
- 適切な活用のタイミングを考える<br />（例えば閲覧後すぐに関連メールが届くと多くのユーザーは気持ち悪さを感じてしまうかもしれない）
- 社会的な優劣の評価にならないようなアウトプットにする
...
さらに「安心安全のデータ活用」のためには使うべきではない … 例えばリファラやクエリパラメータから誤って混入したり、削除が漏れてしまった個人データの検知を仕組化することをお勧めします（[参考](https://techblog.lycorp.co.jp/ja/20231101a)）。
