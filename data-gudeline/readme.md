...
ただし、記事の内容が全てではないことにご注意ください。法律観点はもとより、データのライフサイクル観点でも「保管」や「破棄」についての記載は不十分です。それをふまえ、各章ごとに設けた「Check !」の項目を、データ活用におけるスクリーニングの観点として参考にしてもらえますと幸いです。
...
また、今後該当するユースケースの増加を見込んでいる場合、ガバナンスの強化と効率化を目的として記録の内容、保持期間、そして開示請求への対応について標準化～仕組化することを検討してみましょう。余談ですが私も関連する社内プラットフォームの開発を担当しており、仲間も [募集中](https://www.lycorp.co.jp/ja/recruit/career/job-categories/ly00056/) です。
...
いかがでしたでしょうか。この記事を通じてみなさまの「安心安全のデータ活用」に少しでも貢献できれば幸いです。蛇足になりますが、グローバルな多言語の協業体制の場合、この記事で用いたレベルの用語（例えば第三者提供 = provision to a third party など）については英訳をチーム内で共有しておくことで、コミュニケーションの円滑化（認識齟齬の削減）につながります。
