...
最後に攻撃者が SMS を送信するケース（SMS を使ったフィッシングをスミッシングと呼ぶのだそう）について考えてみます。SMS の場合は送信者 ID 毎にメッセージがスレッド表示されますが、攻撃者が送信者 ID を詐称することで、正規のサービス事業者と同じスレッドに攻撃者のメッセージが表示される可能性があります。
