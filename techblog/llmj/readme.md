...
最初に評価プロセス全体像を示します。プロセスに登場するのは熟練編集や運用担当などの人間（緑）と評価ロボット（青）と生成パイプライン（赤）です。
...
生成パイプラインは、最終的にサービスのプロダクトに実装されることになります。従って、フレームワーク内で扱うプロンプトやルールベースの処理は、プロダクトに対する実装仕様と位置付けることができます。

最初から高い完成度を目指して生成パイプラインを作りこんでもよいですが、品質定義からナイーブなプロンプトを生成する仕組みを用意したので、初版はそれを用いることにします。
...
プロンプトの指示は、常に守られるという保証はないため、ルールベースの処理はプロンプトから分離して
...
生成パイプラインの品質が一定程度高くなった後に、十分な量と多様性を持つ入力データセットに切り替えることで、効率よく改善を進めることができます。
