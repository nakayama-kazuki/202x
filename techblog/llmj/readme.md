...
こんにちは、広告、データプラットフォームを経て現在はメディアエンジニアの中山です。少し前に Google の AI Overview が <a href='https://blog.google/products-and-platforms/products/search/ai-overviews-update-may-2024/'>ピザにチーズをくっつけるために接着剤を使う</a> アドバイスを掲出して話題になりました。そして、今日では多くのサービスで生成 AI が活用されるようになってきましたが、皆さんは生成 AI の出力品質にどのように向き合ってますか？

私の担当するサービスでも、ニュース記事や SNS ポストのブリーフィングを自動生成したいというニーズがありました。そこで、この記事ではブリーフィングの自動生成を題材に、
...
評価ロボットの実装方法はいろいろ考えられますが、例えば <a href='https://deepeval.com/'>DeepEval</a> を利用することで、2 の結果を入力とする評価の仕組自体は容易に実装できます。ですが、それだけでは実務に落とし込む上で十分とは言えません。ここからは発生した課題とその対策について、各ステップ毎に掘り下げていきます。
