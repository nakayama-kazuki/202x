...
では「よいブリーフィング」の具体例と品質定義の整合性に加え、評価観点の重複や衝突も確認することで、評価ロボットの改善を促します。

この段階を通じて、人間と評価ロボットの双方にとっての「よいブリーフィング」に対する解像度を高めていきます。
...
でランダムに生成した入力データセットを使い、同じ評価を繰り返すことで「評価のブレ」の傾向を把握できるようにしました。
...
生成パイプラインは最終的にサービスのプロダクトに実装されます。従って、フレームワーク内で扱うプロンプトやルールベースの処理は、プロダクトに対する実装仕様と位置付けることができます。

これらは、最初から高い完成度を目指して作りこんでもよいのですが、品質定義からナイーブなプロンプトを生成する仕組みも用意したので、初版はそれを用いてみます。
...
これらはプロンプトで禁止を指示しても、常に守られるという保証はありません。

なのでプロンプトによる指示とルールベースの処理を分離して

1. プロンプトによる生成
2. ルールベースの処理で上記 1 の結果を加工および確認
3. 確認の結果問題があれば上記 1 を再実行。この際に失敗を繰り返さないための調整を適用
	- 失敗事例と失敗理由を添えた改善要求をプロンプトに追加する
	- パラメータ（temperature）を一時的に変更し出力候補の多様性を高める

のように生成パイプラインを実装しています。

将来的にはより複雑なパイプラインを扱いたくなるかもしれませんが、それはニーズが生じたときに考えることにします。

なお、評価プロセスの序盤では、入力データセットの件数を抑えて改善サイクルの速度を優先することをお勧めします。

生成パイプラインの品質が一定程度高くなった後に、大量かつ多様なデータセットに切り替えることで、効率よく改善を進めることができます。
