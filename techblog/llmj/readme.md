...
多様性を調整する `temperature` パラメータに着目すると、論文 <a href='https://aclanthology.org/2023.emnlp-main.153.pdf'>G-EVAL: NLG Evaluation using GPT-4 with Better Human Alignment</a> では GPT-3.5 の評価において、モデルの決定性を高めるため `temperature` を 0 としました。
...
また DeepEval の `AnthropicModel` でも <a href='https://deepeval.com/integrations/models/anthropic#in-code'>0 をデフォルト</a> にしています。
...
こうすることで、生成パイプラインの改善方針や、デプロイ判定の根拠を強化しました。

# 改善サイクルを反復する

ここまでの取り組みを反映し、評価ロボットはこのようなレポートを出力します。
...
ここで、トレースバックが出力されると改善サイクルは止まってしまうため、DeepEval 内部で発生する例外について踏み込んだ対応を行いました。
...
ニュース記事のタイトルには、その媒体の意図（例えば何を強調したいか）が含まれる場合がありますが、

生成パイプラインの出力が意図の影響に引っ張られて、事実誤認寄りのブリーフィングが生成されてしまうことがありました。

このケースでは、評価ロボット目線では入力と出力の整合性は保たれているため、正確性の評価では問題を検知することができませんでした。

タイトルを入力から除外することで問題は解消できましたが、評価モデルや品質定義だけでなく、入力データも評価結果を左右するということには留意が必要です。
