...
2. 続いて、具体例を抽象化し「よいブリーフィング」たる評価基準を複数の観点から策定してもらい、これを評価ロボットの頭脳とする
3. ここで、検証目的の評価を行う。評価ロボットは「よいブリーフィング」の具体例を「よい」と評価できるだろうか？
4. 期待した検証結果に至るまで、具体例もしくは評価基準を調整する
...
ここで「どの短文が一番よいか」と問われた場合、何をもって「よい」とするのかの評価基準を抜きにしては、再現性のある回答はできません。
...
などの共通する特徴を抽出し、評価基準に落とし込んでいきます。

# 評価基準の策定

DeepEval には、出力が入力に忠実であるかを評価する `Faithfulness` や、要約としての品質を評価する `Summarization` などの既成 Metrics が用意されています。しかし「よいブリーフィング」たる評価基準が、必ずしも既成 Metrics に対応しているとは限らないため、今回は自然言語で評価基準を策定できる <a href='https://deepeval.com/docs/metrics-llm-evals'>G-Eval</a> を利用することにします。
...
といった具合に評価基準を策定し、評価ロボットはこれらを参照します。

複数観点の評価を並列に実行するため、観点を増やしても処理時間への影響は抑えることができますが

- 観点を横断して評価基準が重複し、評価基準それ自身の保守性が悪化する
- 観点を横断して評価基準が衝突し、例えば「簡潔さ」の改善が「網羅性」の改悪を招くなど、生成パイプラインの改善に支障が出る
...
では「よいブリーフィング」の具体例と評価基準の整合性に加え、評価基準の重複や衝突もレポートすることで、評価ロボットの改善を促すようにしました。
...
最初から高い完成度を目指して作りこんでもよいのですが、初版は修正影響を把握しやすいシンプルなプロンプトをお勧めします（フレームワークでも、評価基準からシンプルなプロンプトを自動生成するツールを用意しました）。
...
ところで、条件分岐や多段プロンプトへの対応は将来の検討でよさそうですが、メタ情報の活用については既にニーズがあるようでした。例えば 40 代男性と 10 代女性では、同じニュースに対しても着眼点は異なるかもしれません。また、レストランに関する SNS ポストの場合、ブリーフィングを掲出するタイミングの天気や時間帯によって、訴求軸を変えたくなるかもしれません。

そこで、メタ情報もプロンプト内で扱う場合、評価ロボットも G-Eval の `SingleTurnParams.CONTEXT` を有効化し、生成と評価の条件をそろえるようにしています。
...
スコアに基づく意思決定のためには、評価のブレを抑制しつつ、その傾向も理解した上で変化を解釈する必要がありそうです。まずは、ブレの抑制を試みます。
...
また DeepEval の `AnthropicModel` でも <a href='https://deepeval.com/integrations/models/anthropic#in-code'>デフォルトは 0 です</a>。
...
加えて G-Eval には、生成 AI が出力するスコア候補の確率を利用して加重平均を求め、スコアリングの <a href='https://deepeval.com/docs/metrics-llm-evals#how-is-it-calculated'>バイアスを抑える仕組み</a> があります。直接的にブレを抑制できるわけではありませんが、安定したスコアリングへの寄与を期待して、この仕組みも利用することにしました（ただし Bedrock など一部のバックエンドでは、スコア候補の確率を利用することができません）。
...
さらに、G-Eval が内部的に `evaluation_steps` という <a href='https://deepeval.com/docs/metrics-llm-evals#how-is-it-calculated'>段階的なタスクを生成</a> して評価を実行する点にも着目します。
...
ところが、バッチ処理の都度段階的なタスクを生成すると、前バージョンと現バージョンの評価で、異なる `evaluation_steps` が生成され、結果として評価のブレが生じる懸念があります。

そこで、`evaluation_steps` は評価基準が更新されない限り、同じものを再利用する実装にして評価のブレを抑制しました（副次的に処理コストも下げることができました）。

# ブレの傾向を理解

ここまでブレの抑制を試みましたが、それでも完全に抑制することはできません。さらに、評価観点が離散的かつ客観的である場合（事実との整合性など）と、連続的かつ主観的である場合（ブリーフィングの訴求力など）では、評価のブレ幅が異なったものになることが予想できます。

そこで、ブレの傾向を理解して、スコアに基づく意思決定のヒントにしたいと思います。
...
評価ロボットは、`stddevAvg` や `stddevMax` から把握した評価の安定性も参考にしつつスコア変化を解釈し、改善や副作用（デグレ）について定性的なフィードバックを出力します。

なお `testDataInfo.stddev` は評価のブレではなく、入力データセットに対する評価のスコアがどの程度幅広く分布しているかを見るための指標です。ここに問題があれば、入力データセットを見直して傾向を再度取得します。

これで、生成パイプラインの改善方針や、デプロイ判定の根拠を強化することができました。

# 改善サイクルを反復する

評価ロボットは、ここまでの取り組みを反映したレポートを出力します。
...
このレポートを参考に、改善サイクルを反復するのですが、評価ロボットがトレースバックを出力して停止してしまうと改善も止まってしまいます。それを回避するため DeepEval 内部で発生する例外について踏み込んだ対応を行いました。
...
そこで、やや苦しい対応ながらも、モデルが JSON として解釈できない出力を返した場合は、リトライすることで回避しました。
...
続いて、改善サイクルの反復により得た気付きをご紹介します。

ニュース記事のタイトルには、その媒体の意図（例えば何を強調したいか）が含まれる場合がありますが、生成パイプラインの出力が意図の影響に引っ張られて、事実誤認寄りのブリーフィングが生成されてしまうことがありました。このケースでは、評価ロボット目線では入力と出力の整合性は保たれているため、正確性の評価では問題を検知することができませんでした。結局、タイトルを入力から除外することで問題は解消できましたが、評価モデルや評価基準だけでなく、入力データも評価結果を左右するということには留意が必要です。

また、個別の事実は正確でも、関係性を間違えたブリーフィングを出力してしまうケースもしばしば発生しました。このケースについては、正確性の評価基準に「主体」「対象」「出来事」「条件」などの関係性維持の指示を追加し、改善を試みました。

なお、改善サイクルの序盤では、少ない入力データセットでも問題を見つけやすいため、スピードを優先することをお勧めします。生成パイプラインの出力品質が一定程度高くなった後に、十分な量と多様性を持つ入力データセットに切り替えることで、全体として効率よく改善を進めることができます。

# おわりに

最後に本番運用についても補足します。

ニュース記事のブリーフィングでは、皇室に関する話題や著名人の自殺報道など、リスクを受容できないケースもあるかもしれません。このようなケースでは、自動生成の対象外とするか、HITL による掲出前の目視が対策として考えられます。また、本番プロダクト側でも評価ロボットを動かし、リスクを検知した時には必要に応じて差し止める運用機能が必要になるかもしれません。

長くなりましたが、この記事が生成 AI の出力品質に関する悩みを解決するヒントになれば幸いです。
...
あわせて、少し前に <a href='https://blog.techscore.com/entry/2026/05/13/080000_1'>個人開発の AI 連携アプリに関する記事</a> を書いたのですが、よろしければそちらもご覧ください。結びで用意した伏線を、今回の記事で無事回収することができました 😊
