...
생성 파이프라인의 출력이 그 의도에 지나치게 끌려가면서 사실을 잘못 전달하는 브리핑이 생성되는 경우가 있었습니다.
...
또 개별 사실 자체는 정확하지만, 사실 간 관계를 잘못 연결한 브리핑의 문제를 탐지하지 못하는 경우도 있었습니다.

이 경우에는 "주체", "대상", "사건", "조건" 등의 관계가 유지되는지도 평가할 수 있도록 정확성 Rubric을 수정해 개선을 시도했습니다.

또한 SNS 포스트에는 오타나 잘못된 정보가 포함될 수 있지만, 브리핑을 자동 생성하는 과정에서 이를 임의로 수정할 수는 없습니다. 이런 경우에는 오타나 잘못된 정보일 가능성이 있는 내용에 의존하지 않고, 해당 내용을 사용하지 않아도 표현할 수 있는 수준까지 추상화해야 합니다.

마지막으로 운영 측면에서 얻은 인사이트입니다.
