...
さて、試行錯誤を重ねた WAF には後日談がありまして、診断アプリのトラフィック規模では、WAF の固定費が Bedrock の従量課金額を上回ってしまいました。見積もらなければこうなるよ、という身を挺した教訓となります ^^; 今後については、トラフィックが伸びるまでは WAF の利用を中断し、POST 回数の制限を DynamoDB で実装することも検討中です。
