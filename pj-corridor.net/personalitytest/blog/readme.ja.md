...
ボットを高度化すればこれらの対策は迂回可能ですが、初手としては基本方針にもとづいた対策に留めることにします。アクセス状況をモニタリングしつつ、必要に応じて追加の対策を検討すましょう。
...
Lambda の法式が決まりましたが、ここでアプリケーション開発に進みたい気持ちをぐっとこらえ、後で楽をするために Lambda 環境とテスト環境を透過的に扱うための仕組を用意します。
...
のような状況から、現段階ではコストがメリットを上回るため API Gateway へのオフロードは見送りとしました。
...
のようにチューニングを進めました。診断アプリのように多言語をサポートしている場合、念のために 3 の段階で全ての言語で生成 AI 出力を確認してみましょう（内容は翻訳して確認）。私はこの段階で `hi` `ko` `zh` の処理が失敗していることに気づき、同じ内容でも、生成に必要なトークン数は言語によって大きく異なる 😮 ことを知りました。また、Gemini や ChatGPT のアドバイスは保険的な指示の追加でプロンプトを肥大化させる傾向があるので、適宜指示の統廃合や、文書構造のリファクタリングに取り組むことをお勧めします。
