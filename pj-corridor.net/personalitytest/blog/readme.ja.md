...
のようにチューニングを進めました。診断アプリのように多言語をサポートしている場合、念のために 3 の段階で全ての言語で生成 AI 出力を確認してみましょう（内容は翻訳して確認）。私はこの段階で `hi` `ko` `zh` の処理が失敗していることに気づき、同じ内容でも、生成に必要なトークン数は言語によって大きく異なることを知りました。また、Gemini や ChatGPT のアドバイスは保険的な指示の追加でプロンプトを肥大化させる傾向があるので、適宜指示の統廃合や、文書構造のリファクタリングに取り組むことをお勧めします。
