...
診断アプリは匿名でアクセスできますが、バックエンドには課金の発生する生成 AI（Amazon Bedrock）を利用しています。そのため、ボットによるアビューズで爆死、というのは想定しうるシナリオです。とはいえ「リスクの排除」ではなく、攻撃側のインセンティブも考慮し「顕在化リスクの小さい脅威は受容し、それ以外は遮断」を基本方針としてアビューズ対策を実装しました。
...
を採用し、診断アプリに必要のないルールは削除しました。ただ、Lambda 関数 URL を公開エンドポイントとして利用する場合、CloudFront を経由しないリクエストが WAF をバイパスできてしまうことが懸念事項です。CloudFront 経由（= WAF 経由）であることを担保する方法もありますが、
...
3. サーバは人間の回答時間の範囲で生成され得るトークンならば正常なクエリと判断し、それ以外は遮断
...
ボットを高度化すればこれらの対策は迂回可能ですが、初手としては基本方針にもとづいた対策に留めることにします。アクセス状況をモニタリングしつつ、必要に応じて追加の対策を検討しましょう。
...
については、当初 1 が魅力的な選択肢でした。もともとローカルに PHP のテスト環境を構築済みだったので、アジャイルな開発～テストに取り組めると考えていたからです。しかし Lambda との親和性や CI の複雑化の懸念もふまえ、結果として 2 を採用することにしました。振り返ってみれば、AWS 環境での試行錯誤やブラックボックスを紐解く時間の方が相対的に長かったため、的を射た選択だったかと思います。

試行錯誤といえば、AWS コンソールから Lambda 関数の作成を繰り返すと、その都度新しい IAM Role が自動生成されます。IAM Role の残骸に限らず、不要なリソースを放置すると将来の技術負債になるので、忘れないうちに削除しておきましょう。
...
2. テスト環境（生成 AI 接続なし）でのチューニング
	...
	3. プロンプトを普段使いの Gemini や ChatGPT に入力し（本番をシミュレート）出力を評価
...
のようにチューニングを進めました。診断アプリのように多言語をサポートしている場合、本番環境では全ての言語で生成 AI 出力を確認してみましょう。私の場合、この段階で `hi` `ko` `zh` に限って出力の異常があることに気付くことができました。同じ内容の文章生成であっても、必要なトークン数は言語によって大きく異なるようです（例えば `hi` は `en` の 3 倍程度 😮）。

また、入力についても Gemini や ChatGPT のアドバイスは保険的な指示の追加でプロンプトを肥大化させる傾向があるので、適宜指示の統廃合や、構造のリファクタリングに取り組むことをお勧めします。
...
と指示した場合、文法上の主語が「あなた」になってしまいます。そこで代名詞の利用禁止の指示を加えると、今度は「Respondent」に 🤔 なってしまいます。これは生成 AI がより自然な表現を優先するためで、指示のチューニングによって安定的に「回答者」と出力させることは難しく（これはこれで学びですが）、最終的には
