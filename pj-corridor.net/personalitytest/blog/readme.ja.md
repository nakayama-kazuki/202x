...
診断アプリは匿名でアクセスできますが、バックエンドには課金の発生する生成 AI（Amazon Bedrock）を利用しています。そのため、ボットによるアビューズで爆死、というのは想定しうるシナリオです。とはいえあらゆる脅威の排除を目指すのではなく、攻撃側のインセンティブも考慮して「顕在化リスクの小さい脅威は受容し、それ以外を対策」を基本方針としました。
...
については、当初 1 を優先したい気持ちがありました。もともとローカルに PHP のテスト環境を構築済みだったので、アジャイルな開発～テストに取り組めると考えていたからです。しかし Lambda との親和性や CI の複雑化の懸念もふまえ、結果として 2 を採用することにしました。振り返ってみれば、AWS 環境での試行錯誤やブラックボックスを紐解く時間の方が相対的に長かったため、的を射た選択だったかと思います。
...
のようにチューニングを進めました。診断アプリのように多言語をサポートしている場合、本番環境では全ての言語で生成 AI 出力を確認してみましょう。私の場合、この段階で `hi` `ko` `zh` の出力に限り欠損が生じていることに気付くことができました。同じ内容の文章生成であっても、言語によって必要なトークン数は大きく異なるようです。
