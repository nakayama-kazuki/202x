...
のようにチューニングを進めました。診断アプリのように多言語をサポートする場合、念のために 3 の段階で全ての言語で生成 AI 出力を確認してみましょう（内容は翻訳して確認）。私はこの段階で幾つかの言語で処理が失敗していることに気づき、言語によって必要になるトークン数が異なることを知りました。また、Gemini や ChatGPT のアドバイスは、プロンプトを肥大化させる傾向があります。適宜指示の統廃合や、文書構造のリファクタリングにも取り組むことをお勧めします。
