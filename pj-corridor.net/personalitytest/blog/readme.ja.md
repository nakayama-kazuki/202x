...
アドバイスと取り扱い説明書は生成 AI による出力ですが、日本語の文章中に現れる「Respondent」に唐突感がありますね。このように生成 AI 連携アプリは高い表現力を得る一方で品質面のリスクが生じます。とはいえ、品質担保にかけるコストは趣味プログラミングの範囲に抑えたいところです。そこで、ルールベースの採点ロジックは deterministic に保ち、生成 AI の出力は補足的な文章に留めることでハルシネーションの影響範囲をコントロールしました。

それでは、ここから具体的な試行錯誤やトレードオフへの向き合い方について 3 つの章に分けてご紹介します。

## 1. アビューズ対策編

診断アプリは匿名でアクセスできますが、バックエンドには課金の発生する生成 AI（Amazon Bedrock）を利用しています。そのため、ボットによるアビューズで爆死、というのは想定しうるシナリオです。とはいえあらゆる脅威の排除を目指すのではなく、攻撃側のインセンティブも考慮して「顕在化リスクの小さい脅威は受容し、それ以外は対策」を基本方針としました。
...
いずれにしても AWS 環境に依存した実装が必要になります。悪意ある第三者が Lambda 関数 URL を知り、WAF を迂回するリスクを勘案し、今回の対応は見送りとしました。
...
については、当初 1 を優先したい気持ちがありました。もともとローカルに PHP のテスト環境を構築済みだったので、アジャイルな開発～テストに取り組みやすいと考えていたからです。しかし Lambda との親和性や CI の複雑化の懸念もふまえ、結果として 2 を採用することにしました。振り返ってみれば、AWS 環境での試行錯誤やブラックボックスを紐解く時間の方が相対的に長かったため、的を射た選択だったかと思います。

試行錯誤といえば、AWS コンソールから Lambda 関数の作成を繰り返すと、その都度新しい IAM Role が自動生成されます。その残骸に限らず、使わないリソースを放置すると将来の技術負債となるため、忘れないうちに削除しておきましょう。

さて、実行環境の準備はできましたが、ここでアプリケーション開発に進みたい気持ちをぐっとこらえ、後で楽をするために Lambda 環境とテスト環境を透過的に扱うための仕組を用意します。
...
3. 本番環境（生成 AI 接続あり）でのチューニング
	1. ショートカット機能を用いてデグレ部分チューニング
	2. 診断アプリを通しで実行し、出力内容の違和感などを最終チェック

のようにチューニングを進めました。診断アプリのように多言語をサポートしている場合、本番環境では全ての言語で生成 AI 出力を確認してみましょう。私の場合、この段階で `hi` `ko` `zh` の出力に限って欠損が生じていることに気が付きました。同じ内容の文章生成であっても、言語によって必要なトークン数は大きく異なるようです。
...
と指示した場合は「あなた」になってしまいます。そこで代名詞の利用を禁止する指示を加えると、今度は「Respondent」になってしまいます 🤔。これは、学習データに基づく自然な表現の生成と、主語や参照語に関する指示が競合していることが原因で、最終的に出力を安定させるために
