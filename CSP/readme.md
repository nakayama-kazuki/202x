...
ところが、Web 解析ツールや広告のビューアビリティー計測など 3rd-party JavaScript がその目的を達成するために文書「Ａ」の DOM にアクセスする必要がある場合、この対策を採用することができません。そのような 3rd-party JavaScript については安全性を評価の上リスク受容せざるをえませんが、それ以外の 3rd-party JavaScript に対して CSP を活用してロードと実行が制限された状態を保証する、あたりが現実的でしょうか。
...
などの前提のもとで運用しなければならないためです。過剰な制限のもとではサービスとしての要求事項を満たせず、取りこぼしは潜在的なリスクの増加につながるため、こうなると CSP の許可リストを用意すること自体が難しくなります。
