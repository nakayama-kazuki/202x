...
そこで今回はホワイトボードを活用した議論の代替案として、ポンチ絵を多用したパワーポイントプレゼンテーション（パワポ + ポンチ = パワーポンチと名付けました :-p）と、それを用いた議論についてまとめてみました。
...
1. 配布資料（例えばホワイトペーパー）に最適化
2. 一方通行のプレゼンテーション（例えばキックオフ）に最適化
3. 双方向の議論（例えば新しいアイデアや複雑な問題を扱う際の会議）に最適化

はそれぞれ異なったマテリアルであり、ホワイトボードを活用した議論に近しい効果を期待するならば 3 にコストをかける価値は十分にあるかと思います。
