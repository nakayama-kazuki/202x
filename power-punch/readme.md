...
参加者は些細な質問でプレゼンテーションの腰を折りたくないとは思いつつも、わからない部分を残したままにしておくことにストレスを感じ、最終的に集中力も低下します。そこで論点が切り替わるタイミングで、それとわかるような（例えば通常のスライドに対して前景色と背景色を入れ替えた）スライドを挿入し、同時に進捗状況も可視化してみましょう。
...
例えば、ステップ毎にこのような気付きがありそうです。

1. 残像を使ったパラパラまんが風ポンチ絵で、参加者のメンタルモデルに「ＡとＢの統合」を訴えかけます。すると、参加者はＡやＢに紐づく属性情報や状態のコンフリクトが気になり、同時にそれを解消するための創造性を発揮します。
2. Ｃのみ特別な扱いが必要になったことをその背景も含めて説明します。参加者は背景は理解しつつ、その関係性を UI で表現することが困難であることに思い至ります。
3. ＤやＥの扱いもＣ同様にして、全体として一貫性を持たせた案について説明します。参加者は Before & After についての理解が深まりましたが、同時により良い方法について模索を開始します。

ところで、参加者の誰かがペンツールを使い始めると、他の参加者は画面を眺めつつその完成を待ちます。特別なデバイスの支援を抜きで壮大な作品にチャレンジした場合、それなりに時間をロスしてしまうことと、仕上がりが芸術的すぎて誰も理解できない、という残念な事態がしばしば発生します。

<img src='https://raw.githubusercontent.com/nakayama-kazuki/202x/main/power-punch/img/pen.png' />

ペンツールは視覚的な刺激を交えて双方向性を促進しますが、
